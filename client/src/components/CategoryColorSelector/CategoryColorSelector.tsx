...
const CategoryColorSelector = ({
  ...
  const handleCategoryChange = (e) => {
    ...
    if (categories[selectedCategory]) {
      setColor(categories[selectedCategory].color); // Set the color based on the selected category
    } else if (selectedCategory !== "") { // Only clear color and category if a value was provided
      console.error("Selected category does not exist:", selectedCategory);
      setCategory(''); 
      setColor(''); // Clear color if invalid selection
    }
  };
  ...
  return (
    <div>
      <FormControl variant="outlined" fullWidth>
        ...
        >
          ...
          {Object.keys(categories).map((key) => (
            <MenuItem key={key} value={key} style={{ color: categories[key].color }}>
              {categories[key].name} 
            </MenuItem>
          ))}
        </Select> 
      ...
      <div style={{ marginTop: '20px' }}>
        <Button 
          sx={{ borderRadius: '30px', backgroundColor: 'var(--orange-color)', color: "var(--black-color)" }} 
          variant="contained"
          onClick={handleClick}
        >
          Select Color
